...
async function syncOwnRecipesFromFirestore() {
  ...
  if (user && user[0].email) {
    const querySnapshot = await firestore
      .collection("recipes")
      .where("author", "==", user[0].email)
      .get();
    const querySnapshot2 = await firestore
      .collection("recipes")
      .where("author", "==", user[0].displayName)
      .get();
...
async function syncFileInDBs(firestoreDoc, idbDoc) {
  if (firestoreDoc && firestoreDoc.date_edit) {
    if (
      idbDoc.date_edit &&
      idbDoc.date_edit.seconds &&
      firestoreDoc.date_edit.seconds &&
      idbDoc.date_edit.seconds > firestoreDoc.date_edit.seconds
    ) {
    ...
    if (ids.favorites)
      await Promise.all(
        ids?.favorites.map((id) =>
          getFromFirestore(id).then(syncFirestoreDocToIndexedDB)
        )
      );
...
async function syncFavoritesList(ids) {
  ...
  let idbEditDate = idbUser
    ...
    : {};
  if (idbUser) {
    if (arraysEqual(idbFavorites, newFavorites) && newFavorites.length > 0)
      return;
  ...
  if (
    !idbUser[1] ||
    !idbUser[1].favorites ||
    (idbUser[1].favorites.length < 1 && newFavorites.length > 0) ||
    !arraysEqual(idbUser[1].favorites, newFavorites)
  ) {
  ...
  if (idbUser[0] && idbUser[0].displayName && !arraysEqual(ids, newFavorites)) {
    await addFavoritesListToFirestore(
      idbUser[0].displayName,
      newFavorites,
      newEditDate
    );
  }
...
async function syncImages(id) {
  ...
  if (object.metadata) {
    ...
    metadata.updated
      ? (storageLastEdit = firebase.Timestamp(new Date(metadata.updated)))
      : null;
  }
  if (
    (!storageLastEdit || indexedDBImage.last_edit > storageLastEdit) &&
    indexedDBImage
  ) {
...
async function convertImageToBlob(url) {
  return fetch(url)
    ...
    .then((blob) => {
      return { id: id, image: blob, date_edit: firebase.Timestamp.now() };
    });
}
...
// Message to firestore.ts as the service worker can't do these requests
...
function sendMessageToClient(message) {
  return clients.matchAll().then((clients) => {
    if (clients && clients.length) {
      ...
      return Promise.reject("No active client found.");
    }
  });
}
